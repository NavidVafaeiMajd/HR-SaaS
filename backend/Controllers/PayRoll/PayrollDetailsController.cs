...
public class PayrollDetailsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetEmployeePayrollDetails(string userId)
    {
        ...
        var historySalary = salary.History
            .Where(x => IsEffective(x.EffectiveYear, x.EffectiveMonth, currentYear, currentMonth))
        ...
        var useCurrentSalary = currentSalaryIsEffective
            && (historySalary == null
        ...
        if (!useCurrentSalary && historySalary == null)
            return NotFound("تا تاریخ امروز، حقوق مؤثری برای این کارمند ثبت نشده است.");
        ...
        var paidPayments = payments.Where(x => x.status == PayrollPaymentStatus.Paid).ToList();

        return Ok(new
        {
            asOf = new { year = currentYear, month = currentMonth },
            currentSalary = new
            {
                ...
                effectiveSalary.effectiveYear,
                effectiveSalary.effectiveMonth,
                effectiveSalary.source,
                totalAllowances = effectiveSalary.housingAllowance + effectiveSalary.foodAllowance
                    + effectiveSalary.transportationAllowance + effectiveSalary.childAllowance
                    + effectiveSalary.seniorityAllowance,
                grossSalary = effectiveSalary.baseSalary + effectiveSalary.housingAllowance
                    + effectiveSalary.foodAllowance + effectiveSalary.transportationAllowance
                    + effectiveSalary.childAllowance + effectiveSalary.seniorityAllowance
            },
            paymentSummary = new
            {
                paidCount = paidPayments.Count,
                paidAmount = paidPayments.Sum(x => x.netSalary),
                totalAllowances = paidPayments.Sum(x => x.totalAllowances),
                totalDeductions = paidPayments.Sum(x => x.totalDeductions),
                pendingCount = payments.Count(x => x.status == PayrollPaymentStatus.Pending),
                cancelledCount = payments.Count(x => x.status == PayrollPaymentStatus.Cancelled)
            },
            paidPayments,
            salaryHistory = salary.History
                ...
                .Concat(new[]
                {
                    new
                    {
                        ...
                        createdAt = salary.UpdatedAt ?? salary.CreatedAt,
                        recordType = "Current"
                    }
                })
                .OrderByDescending(x => x.effectiveYear)
