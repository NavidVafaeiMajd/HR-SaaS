...
const ClientPage = () => {
  ...
  const useGetEmployee = async (): Promise<any> => {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_API_BASE_URL || "http://localhost:8000/api"}/clients/${id}`,
            ...
            <div className="flex justify-between p-5 items-center">
              ...
              <div>
                {queryData?.status === "غیرفعال" ? (
                  <span className="bg-red-200 text-red-500 py-1 px-4 rounded-sm text-sm!">
                    غیرفعال
                  </span>
                  ...
                  <span className="bg-amber-200 text-yellow-600 py-1 px-4 rounded-sm text-sm!">
                    تعیین نشده
                  </span>
                )}
              </div>
