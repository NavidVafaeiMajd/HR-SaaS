...
const MarketingStaffDetails = () => {
  ...
  const useGetMarketingStaff = async (): Promise<any> => {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000/api'}/marketing-staff/${id}`);
    if (!res.ok) {
          ...
          <div>
            <div className="flex justify-between p-5 items-center">
              <div className="flex gap-3">
                <div>
                  {queryData?.image ? (
                    <img className="w-25 h-25 object-cover rounded-full" src={`${import.meta.env.VITE_API_BASE_URL?.replace('/api', '') || 'http://localhost:8000'}/${queryData?.image}`} alt="" />
                  ) : (
