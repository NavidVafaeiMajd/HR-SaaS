...
export function DataTable<TData extends Record<string, unknown>, TValue>({
  ...
  const {
   ...
   const dataForTable = useMemo(() => {
     // If there's a search query, use filteredData (even if empty)
     // If no search query, use original rows
     if (searchQuery.trim()) {
       return filteredData.slice().reverse();
     }
     return rows.slice().reverse();
   }, [filteredData, rows, searchQuery]);

   useLoadingState(isLoading, "در حال بارگذاری جدول...");
